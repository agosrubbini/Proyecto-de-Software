...
def list_billings():
    ...
    if start_date_str:
        ...
        billings_query = billings_query.filter(
            Billing.payment_date >= start_date)

    if end_date_str:
        end_date = datetime.strptime(end_date_str, '%Y-%m-%d')
        billings_query = billings_query.filter(
            Billing.payment_date <= end_date)

    billings_pagination = billings_query.paginate(
        page=page, per_page=per_page, error_out=False)
    billings = billings_pagination.items

    pos = 1
    for billing in billings:
        billing.employee_name = get_person_name_and_last_name(
            billing.employee_id)
    ...
    if q:
        billings = [billing for billing in billings if q.lower()
                    in billing.employee_name.lower()]

    if payment_method:
        billings = [billing for billing in billings if payment_method.lower(
        ) in billing.payment_method.lower()]
